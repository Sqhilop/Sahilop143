...
api_id = '22316661'
api_hash = 'afd3e0a31084b6ffa226013527c49e54'
bot_token = '6899588941:AAGp3DK3ELgaQHnHbB3HSMes0HajDtbHTmY'

# List of allowed user IDs
allowed_user_ids = ["2104190309", "1048241028"]  # Add your subscribers' Telegram user IDs here
...
async def start(event):
    welcome_message = "Welcome To NARUTO VIP DDOS thanks for using our bot service! Type /attack to start further."
...
async def attack(event):
    ...
    if user_id not in allowed_user_ids:
        await event.respond("Sorry, you are not authorized to use this bot contact @narutoppking for authorization.")
        return
    ...
    if user_id in user_attack_status:
        ...
        if attack_status['ongoing']:
            await event.respond("Your attack is currently running, kindly wait until it completes cooldown.")
            return
...
async def rattack(event):
    ...
    # Check if the user is allowed to use the bot
    if user_id not in allowed_user_ids:
        await event.respond("Sorry, you are not authorized to use this bot. contact @narutoppking")
        return
...
async def handle_attack(event, user_id, ignore_cooldown=False, is_rattack=False, increase_attack_count=False):
    ...
    await event.respond("Successfully initiated the attack! Powerfull ddos by @narutoppking")
    ...
    try:
        ...
        await event.respond("Attack has been completed powed ddos by @narutoppking!")
